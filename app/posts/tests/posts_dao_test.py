...
class TestPostsDao:
    ...
    def test_get_all_has_keys(self, posts_dao, keys_expected):
        ...
        assert first_post_keys == keys_expected, "Полученные ключи неверные"
    ...
    def test_get_one_check_type_has_correct_pk(self, posts_dao, post_pk):
        ...
        assert post['pk'] == post_pk, "Номер полученного поста не соответствует номеру полученного"
    ...
    def test_get_by_user_check_type(self, posts_dao):
        ...
        assert type(posts) == list, "Результат поиска по пользователю должен быть список"
    ...
    def test_get_posts_by_user(self, posts_dao, poster_name, post_pks_correct):
        """Проверяет, что поиск по пользователю работает корректно"""
